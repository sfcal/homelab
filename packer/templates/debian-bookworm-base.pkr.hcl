...
variable "template_prefix" {
  type        = string
  description = "Prefix for template names (e.g., debian-bookworm)"
}
...
variable "iso_file" {
  type        = string
  description = "Path to the ISO file on Proxmox storage (e.g., local:iso/debian.iso)"
...
locals {
  ...
  template_description = "Debian Bookworm Base (${var.environment}) - Built by Packer on ${timestamp()}"
  // Determine if using iso_file or iso_url
  use_iso_file = var.iso_file != null && var.iso_file != ""
}

// --- Source Definition ---
source "proxmox-iso" "debian-bookworm-base" {
  ...
  // Packer Boot Commands for Debian Preseed
  boot_command = [
    "<esc><wait>",
    "auto url=http://{{ .HTTPIP }}:{{ .HTTPPort }}/preseed.cfg ",
    "priority=critical ",
    "interface=auto ",
    "hostname=debian-packer ",
    "domain= ",
    "<enter>"
  ]
  ...
  // Packer HTTP Server for Preseed files
  http_directory = "http" // Relative path to preseed.cfg
}

// --- Build Definition ---
build {
  name    = "debian-base-${var.environment}" // Build name includes environment
  sources = ["source.proxmox-iso.debian-bookworm-base"]
